defmodule FluxVale.MixProject do
  ...
  defp deps do
    [
      ...
      {:ecto_sql, "3.14.0"},
      {:postgrex, "0.22.4"},
      # Plain Oban (v1's locked version) — first consumer: the token
      # janitor, daily 03:00 UTC cron (settled on #23).
      {:oban, "2.23.0"},
      # Triggers as resource actions — #73's Instance lifecycle is the
      # first consumer; the plain-Oban janitor stays plain.
      {:ash_oban, "0.8.14"},
      {:phoenix_html, "4.3.0"},
  ...
  defp aliases do
    [
      # local_seeds.exs stays dev/local-only (the `local` Cluster row) —
      # seeds.exs is the promotable path; see local_seeds.exs for why.
      setup: [
        "deps.get",
        "ash.setup",
        "assets.setup",
