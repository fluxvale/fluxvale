...
| `apps/platform/lib/flux_vale/billing/dodo.ex` + `dodo/webhook.ex` | Standard Webhooks HMAC verification (constant-time, replay window), checkout client | the `PaymentProvider` behaviour's reference port (ADR-0029); HitPay is the live implementation |
