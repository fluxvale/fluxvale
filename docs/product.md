...
with HTTPS, and pay via prepaid credits metered per second of actual resource
usage.
...
- **Usage metering** (per second, calibrated against Netcup COGS): CPU 0.1
  credits/core-hr, RAM 0.1 credits/GB-hr, storage 0.01
  credits/GB-hr. Storage accrues while an instance is *stopped* (the PVC persists).
- **COGS anchor**: one Netcup RS 2000 G12 (8 dedicated EPYC cores, 16 GB ECC,
  512 GB NVMe) ≈ €21.61/mo actual billed price. The unit economics of the whole product are
  calibrated against this box — see architecture.md's resource budget.
